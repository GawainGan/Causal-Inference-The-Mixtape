...
随机变量的期望值，也称为期望或有时称为总体均值，简单来说就是该变量可能取值的加权平均值，其中权重由每个值在总体中出现的概率给出。假设变量<span>X</span>可以取值<span>x<sub>1</sub></span>，<span>x<sub>2</sub></span>，...，<span>x<sub>k</sub></span>，每个值分别以概率<span>f(x<sub>1</sub>)</span>，<span>f(x<sub>2</sub>)</span>，...，<span>f(x<sub>k</sub>)</span>出现。那么我们定义<span>X</span>的期望值为：
...
期望值的第一个性质是，对于任何常数<span>c</span>，<span>E(c) = c</span>。第二个性质是，对于任何两个常数<span>a</span>和<span>b</span>，那么<span>E(aX+b)=E(aX)+E(b)=aE(X)+b</span>。第三个性质是，如果我们有多个常数，<span>a<sub>1</sub></span>，...，<span>a<sub>n</sub></span>和许多随机变量，<span>X<sub>1</sub></span>，...，<span>X<sub>n</sub></span>，那么以下公式成立：

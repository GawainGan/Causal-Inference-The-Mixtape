...
*注：第二个公式的变化在于：A事件发生在前，B事件发生在后。其发生的前后顺序与第一种情况(A∩B / A) 是颠倒的，因此其得到的值也不一样。

在第二个公式Pr(A|B)中，计算的顺序与Pr(B∣A)中的不同，因为我们是在已知B发生的条件下计算A发生的概率，这与已知A发生的条件下计算B的情况相反，因此结果自然也不相同。这种理解是建立条件概率概念的基础。

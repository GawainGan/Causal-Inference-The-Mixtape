...
我们用来推理因果关系的工具基于概率的基础之上。我们常常会使用到统计学中的数学工具和概念，比如期望和概率。在这本书中，我们最常用的工具之一是线性回归模型，但在我们深入讨论之前，我们需要构建一些简单的符号体系。我们将从求和运算符开始。希腊字母（大写的Sigma）代表求和运算符。设<span>x<sub>1</sub></span>, <span>x<sub>2</sub></span>, ..., <span>x<sub>n</sub></span>是一系列数字。我们可以使用求和运算符来紧凑地写出数字的和：
<p>
    $$ \sum_{i=1}^nx_i \equiv x_1+x_2+\ldots+x_n $$
</p>

字母i被称为求和的索引。有时也会使用其他字母，如j或k，作为求和的索引。下标变量简单地代表一个随机变量x的特定值。数字1和n分别是求和的下限和上限。表达式<span>Σ</span><sub>i=1</sub><sup>n</sup><span>x<sub>i</sub></span>可以用语言描述为“对所有i的值从1到n，求<span>x<sub>i</sub></span>的和”。一个例子可以帮助澄清：

<p>
    $$ \sum_{i=6}^9 x_i= x_6+x_7+x_8+x_9 $$
</p>

求和运算符有三个属性。第一个属性被称为常数规则。正式地说，它是(对于任何常数c)：
...
我们可以应用这两个属性得到以下第三个属性：
<p>
    $$
    \begin{align}
       \text{For any constant } a \text{ and } b: \quad \sum_{i=1}^n(ax_i+by_i) =a\sum_{i=1}^n x_i + b\sum_{j=1}^n y_i
    \end{align}
    $$
</p>

在离开求和运算符之前，注意一些不是这个运算符属性的事情也是有用的。首先，比率的求和不是求和本身的比率。
...
表2.5：偏差之和等于零 (Sum of deviations equalling zero)
...
考虑两个数字序列<span>y<sub>1</sub></span>, <span>y<sub>2</sub></span>, ..., <span>y<sub>n</sub></span>和<span>x<sub>1</sub></span>, <span>x<sub>2</sub></span>, ..., <span>x<sub>n</sub></span>。现在我们可以考虑x和y可能值的双重求和。例如，考虑n = m = 2的情况。那么<span>Σ</span><sub>i=1</sub><sup>2</sup><span>Σ</span><sub>j=1</sub><sup>2</sup><span>x<sub>i</sub>y<sub>j</sub></span> 等于<span>x<sub>1</sub>y<sub>1</sub></span>+<span>x<sub>1</sub>y<sub>2</sub></span>+<span>x<sub>2</sub>y<sub>1</sub></span>+<span>x<sub>2</sub>y<sub>2</sub></span>。这是因为
<p>
    $$
    \begin{align}
